...
#     input:
#         energy=f"{EXPERIMENT_FULL_PATH}{NAME_ENERGY_FOLDER}energy.xvg",
#         m1_energy=f"{EXPERIMENT_FULL_PATH}molecule1/energy.xvg",
#         m2_energy=f"{EXPERIMENT_FULL_PATH}molecule2/energy.xvg",
#         network= f"{EXPERIMENT_FULL_PATH}{NAME_NETWORK_FOLDER}network.pkl"
#     output:
#         energy_csv = f"{EXPERIMENT_FULL_PATH}{NAME_ENERGY_FOLDER}energy.csv"
#     run:
#         import pandas as pd
#         import numpy as np
#
#         print(f"in energy.smk some path is {wildcards.some_path}, energy folder {NAME_ENERGY_FOLDER} and network {NAME_NETWORK_FOLDER}")
#
...
def get_lowest_energy_frames(wildcards):
    ...
    required_indices = df_energy.nsmallest(10, "Binding energy [kJ/mol]").index
...
rule join_plots_lowestE:
    ...
    output:
        joint_plot = f"<outputs_indices>all_view{{view_i}}.png"
    run:
...
#     output:
#         joint_plot = f"{EXPERIMENT_FULL_PATH}simulation/all_view{{view_i}}.png"
#     run:
#         from molgri.images.modifying_images import trim_images_with_common_bbox, join_images
#         modified_paths = [f"{os.path.split(file)[0]}/trimmed_{os.path.split(file)[1]}" for file in input]
#         trim_images_with_common_bbox(input,modified_paths)
...
rule violin_plot_E_distributions:
    input:
        network_energy = f"<outputs_network>network_energy.pkl"
    output:
        violin_plot = f"<outputs_plots>violin_plot_energies.png"
    run:
...
#     input:
#         structure=f"{EXPERIMENT_FULL_PATH}{NAME_PT_FOLDER}structure.gro",
#         trajectory=f"{EXPERIMENT_FULL_PATH}{NAME_PT_FOLDER}trajectory.xtc",
#         structure1=f"{EXPERIMENT_FULL_PATH}{NAME_PT_FOLDER}molecule1.gro",
#         structure2=f"{EXPERIMENT_FULL_PATH}{NAME_PT_FOLDER}molecule2.gro",
#         energy_csv=f"{EXPERIMENT_FULL_PATH}{NAME_ENERGY_FOLDER}energy.csv"
#     output:
#         plot=f"{EXPERIMENT_FULL_PATH}{NAME_PLOTS}ring_vector.png"
#     run:
...
#         energy_csv= f"{EXPERIMENT_FULL_PATH}{NAME_ENERGY_FOLDER}energy.csv"
#     output:
#         plot=f"{EXPERIMENT_FULL_PATH}{NAME_PLOTS}ring_position_lowest_{{N}}.png"
#     run:
#         from molgri.plotting import draw_points
#         from MDAnalysis import Universe
#         import plotly.graph_objects as go
