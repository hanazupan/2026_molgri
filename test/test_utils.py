...
def test_normalising_on_arrays():
...
def test_distance_between_quaternions():
    ...
    # max distance between quaternions is pi/2 and that is between two "perpendicular" quaternions
    q4 = np.array([0, 1, 0, 0])
    q5 = np.array([0, 0, 1, 0])
    q6 = np.array([0, 0, 0, -1])
    np.isclose(np.pi/2, distance_between_quaternions(q1, q4))
    np.isclose(np.pi/2, distance_between_quaternions(q2, q4))
    np.isclose(np.pi/2, distance_between_quaternions(q1, q5))
    np.isclose(np.pi/2, distance_between_quaternions(q2, q5))
    np.isclose(np.pi/2, distance_between_quaternions(q1, q6))
    np.isclose(np.pi/2, distance_between_quaternions(q2, q6))
